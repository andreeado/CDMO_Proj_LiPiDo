...
def set_constraints_circle(problem, schedule, data):
    ...
    # Symmetry Breaking: Fix the first week's period assignments as generated by the circle schedule
    first_week_matches = circle_schedule_weeks[0]
    for match_idx, (t1, t2) in enumerate(first_week_matches):
        if match_idx < n_periods:
            for p in Periods:
                if p == match_idx:
                    problem += x[t1, t2, 0, p] + x[t2, t1, 0, p] == 1
                else:
                    # This match cannot be in other periods
                    problem += x[t1, t2, 0, p] + x[t2, t1, 0, p] == 0
